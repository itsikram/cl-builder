(function ($) {
  $(document).ready(e => {
    ...
    let updatePrice = (subPrice, price) => {
      // display subtotal
      $('.product-pricing-box .price-subtotal-container  .price-subtotal').text(subPrice.toFixed(2))
      // display total
      $('.product-pricing-box .price-total-container  .price-total').text(price.toFixed(2))
      // set total cost
    ...
    let updateDisplay = (height, width) => {
      ...
      let pricePerSqft = parseFloat($('.product-attibute-box #pricePerSqft').val());
      let subTotalPrice = pricePerSqft * totalSqft;
      let totalPrice = pricePerSqft * totalSqft;
      let dimentionText = `${totalHeightIn}" x ${totalWidthIn}" = ${totalSqft.toFixed(2)} ft<sup>2</sup> `;
      // display dimention
      $('.product-attibute-box  .total-size-sqft').html(dimentionText);
      $('.product-attibute-box  .total-size-sqft').attr('data-total-sqft', totalSqft);
    ...
    // change attribute
    $(document).on('change', '.product-attibute-box .dynamic-select:not(.avoid-price)', e => {
      let getAttrCurrentCost = parseFloat($(e.currentTarget).attr('data-cCost'));
      let priceBeforeChange = parseFloat($('#totalCost').val());
      updatePrice(priceBeforeChange - getAttrCurrentCost, priceBeforeChange - getAttrCurrentCost);
      ...
      if (costType == '%') {
        priceAfterChange = priceBeforeChange + ((priceBeforeChange / 100) * selectedAttrVal);

        $(e.currentTarget).attr('data-cCost', (priceBeforeChange / 100) * selectedAttrVal)
        //alert(priceAfterChange)
      } else if (costType == 'sqft') {

        let totalSqft = $('.product-attibute-box  .total-size-sqft').attr('data-total-sqft', totalSqft);
        ...
        let sizeValue = $('.select-size').val();
        let sizeInch = $('.select-size').attr('data-selected-size')
    ...
    $(document).on('change', '.product-attibute-box input', e => {
      ...
      switch (e.target.name) {
        ...
        case 'height-ft':
          ...
          var heightIn = parseInt($('.product-attibute-box input[name="height-in"]').val()) || 0;
          var widhtFt = parseInt($('.product-attibute-box input[name="width-ft"]').val()) || 0;
          var widthIn = parseInt($('.product-attibute-box input[name="width-in"]').val()) || 0;
        ...
        // Height In.
        case 'height-in':
          var heigthFt = parseInt($('.product-attibute-box input[name="height-ft"]').val()) || 0;
    ...
    $('#totalCost').change(e => {
      ...
      // display subtotal
      $('.product-pricing-box .price-subtotal-container  .price-subtotal').text(totalPrice.toFixed(2))
      // display total
      $('.product-pricing-box .price-total-container  .price-total').text(totalPrice.toFixed(2))
    })
    ...
    $('#letterInput').keyup(e => {
      ...
      $('#letter-output').text(letterCharacters ? updatedText : 'ENTER NAME')
      let letterPricePerInch = $('#select-size').val() ? parseFloat($('#select-size').val()) : 0;
    ...
    let currentSize = $('#select-size option:first-child').attr('data-size');
    $('#select-size').attr('data-selected-size', currentSize)

    $('#select-size option').click(e => {
    ...
    $('#select-font option').each(e => {
      let fontName = $('#select-font option:nth-child(' + (e + 1) + ')').attr('value');
      $('#select-font option:nth-child(' + (e + 1) + ')').css('font-family', fontName);
    })
